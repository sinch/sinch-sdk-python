...
class SMSEndpoint(HTTPEndpoint):
    def __init__(self, request_data, sinch):
        ...
        if sinch.configuration.get_sms_authentication_method() == HTTPAuthentication.OAUTH.value:
            self.project_or_service_id = sinch.configuration.project_id
            self.sms_origin = self.sinch.configuration.sms_origin

        elif sinch.configuration.get_sms_authentication_method() == HTTPAuthentication.SMS_TOKEN.value:
            ...
            self.sms_origin = self.sinch.configuration.sms_domain_with_service_plan_id
