...
class NumbersEndpoint(HTTPEndpoint):
    ...
    def process_response_model(self, response_body: dict, response_model: type[BaseModel]) -> BaseModel:
        ...
        """
        try:
            model_instance = response_model.parse_obj(response_body)
            cleaned_data = model_instance.dict(exclude_none=True)
            # Update the internal fields of the existing model instance
            for key in list(model_instance.__fields__):
                if key in cleaned_data:
                    setattr(model_instance, key, cleaned_data[key])
                else:
