...
class Calls:
    ...
    def get(self, call_id) -> GetVoiceCallResponse:
        return self._sinch.configuration.transport.request(
            GetCallEndpoint(request_data=GetVoiceCallRequest(call_id=call_id))
        )

    def update(
        self, call_id: str, instructions: Union[list, List[Instruction]], action: Union[dict, Action]
    ) -> UpdateVoiceCallResponse:
        return self._sinch.configuration.transport.request(
            UpdateCallEndpoint(
                request_data=UpdateVoiceCallRequest(call_id=call_id, instructions=instructions, action=action)
            )
        )

    def manage_with_call_leg(
        self, call_id: str, call_leg: str, instructions: Union[list, List[Instruction]], action: Union[dict, Action]
    ) -> ManageVoiceCallResponse:
        return self._sinch.configuration.transport.request(
            ManageCallEndpoint(
                request_data=ManageVoiceCallRequest(
                    call_id=call_id, call_leg=call_leg, instructions=instructions, action=action
                )
            )
        )
...
class Conferences:
    ...
    def get(self, conference_id: str) -> GetVoiceConferenceResponse:
        return self._sinch.configuration.transport.request(
            GetConferenceEndpoint(request_data=GetVoiceConferenceRequest(conference_id=conference_id))
        )

    def kick_all(self, conference_id: str) -> KickAllVoiceConferenceResponse:
        return self._sinch.configuration.transport.request(
            KickAllConferenceEndpoint(request_data=KickAllVoiceConferenceRequest(conference_id=conference_id))
        )
    ...
    def kick_participant(
        ...
        return self._sinch.configuration.transport.request(
            KickParticipantConferenceEndpoint(
                request_data=KickParticipantVoiceConferenceRequest(call_id=call_id, conference_id=conference_id)
            )
        )

    def manage_participant(
        self, call_id: str, conference_id: str, command: str, moh: str = None
    ) -> ManageParticipantVoiceConferenceResponse:
        return self._sinch.configuration.transport.request(
            ManageParticipantConferenceEndpoint(
                request_data=ManageParticipantVoiceConferenceRequest(
                    call_id=call_id, conference_id=conference_id, command=command, moh=moh
                )
            )
        )
...
class Applications:
    ...
    def get_numbers(self) -> GetNumbersVoiceApplicationResponse:
        return self._sinch.configuration.transport.request(GetVoiceNumbersEndpoint())

    def assign_numbers(
        self, numbers: List[str], application_key: str = None, capability: str = None
    ) -> AssignNumbersVoiceApplicationResponse:
        return self._sinch.configuration.transport.request(
            AssignVoiceNumbersEndpoint(
                request_data=AssignNumbersVoiceApplicationRequest(
                    numbers=numbers, application_key=application_key, capability=capability
                )
            )
        )

    def unassign_number(
        self, number: str, application_key: str = None, capability: str = None
    ) -> UnassignNumbersVoiceApplicationResponse:
        return self._sinch.configuration.transport.request(
            UnAssignVoiceNumberEndpoint(
                request_data=UnassignNumbersVoiceApplicationRequest(
                    number=number, application_key=application_key, capability=capability
                )
            )
        )

    def get_callback_urls(self, application_key: str) -> GetCallbackUrlsVoiceApplicationResponse:
        return self._sinch.configuration.transport.request(
            GetVoiceCallbacksEndpoint(
                request_data=GetCallbackUrlsVoiceApplicationRequest(application_key=application_key)
            )
        )

    def update_callback_urls(self, application_key: str, primary: str = None, fallback: str = None):
        return self._sinch.configuration.transport.request(
            UpdateVoiceCallbacksEndpoint(
                request_data=UpdateCallbackUrlsVoiceApplicationRequest(
                    application_key=application_key, primary=primary, fallback=fallback
                )
            )
        )
    ...
    def query_number(self, number) -> QueryNumberVoiceApplicationResponse:
        return self._sinch.configuration.transport.request(
            QueryVoiceNumberEndpoint(request_data=QueryNumberVoiceApplicationRequest(number=number))
        )
