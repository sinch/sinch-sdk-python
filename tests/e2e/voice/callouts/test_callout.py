...
async def test_conference_callout_async(
    sinch_client_async,
    phone_number,
    voice_origin_phone_number,
    conference_id
):
    conference_callout_response = await sinch_client_async.voice.callouts.conference(
        conference_id=conference_id,
        destination={
            "type": "number",
            "endpoint": phone_number
...
async def test_custom_callout_async(
    sinch_client_async,
    phone_number,
    voice_origin_phone_number
):
    custom_callout_response = await sinch_client_async.voice.callouts.custom(
        destination={
            "type": "number",
            "endpoint": phone_number
        },
