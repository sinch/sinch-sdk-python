...
def step_search_available_numbers(context):
    response = context.sinch.numbers.search_for_available_numbers(region_code="US", number_type="LOCAL")
...
def step_rent_any_number(context):
    context.response = context.sinch.numbers.rent_any(
        ...
        voice_configuration={"type": "RTC", "app_id": "sunshine-rain-drop-very-beautifulday"},
        number_pattern={"pattern": "7654321", "search_pattern": "END"},
    )
...
def step_validate_rented_number(context):
    ...
    assert data.next_charge_date == datetime.fromisoformat("2024-06-06T14:42:42.022227+00:00").astimezone(
        tz=timezone.utc
    )
...
def step_rent_specific_number(context, phone_number):
    context.response = context.sinch.numbers.rent(
        ...
        voice_configuration={"app_id": "sunshine-rain-drop-very-beautifulday"},
    )
...
def step_when_list_phone_numbers(context):
    response = context.sinch.numbers.list(region_code="US", number_type="LOCAL")
    # Get the first page
    context.response = response.content()
...
def step_when_update_phone_number(context, phone_number):
    context.response = context.sinch.numbers.update(
        ...
        sms_configuration={"service_plan_id": "SingingMooseSociety"},
        voice_configuration={"type": "FAX", "service_id": "01W4FFL35P4NC4K35FAXSERVICE"},
...
def step_then_response_contains_phone_details(context, phone_number):
    ...
    assert data.next_charge_date == datetime.fromisoformat("2024-06-06T14:42:42.677575+00:00").astimezone(
        tz=timezone.utc
    )
    assert data.expire_at is None
    assert data.sms_configuration.service_plan_id == "SpaceMonkeySquadron"
...
def step_when_release_phone_number(context, phone_number):
    context.response = context.sinch.numbers.release(phone_number=phone_number)
...
def step_then_response_contains_released_number(context, phone_number):
    ...
    assert data.expire_at == datetime.fromisoformat("2024-06-06T14:42:42.677575+00:00").astimezone(tz=timezone.utc)
