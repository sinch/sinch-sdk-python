...
def mock_sinch_async():
    ...
    sinch.authentication = mock_auth
...
class TestAsyncHTTPTransport:
    ...
    async def test_authenticate(self, mock_sinch_async, base_request, auth_type):
        transport = MockAsyncHTTPTransport(mock_sinch_async)
        endpoint = MockEndpoint(auth_type)
        
        if auth_type == HTTPAuthentication.BASIC.value:
            result = await transport.authenticate(endpoint, base_request)
            assert isinstance(result.auth, httpx.BasicAuth)
            assert result.auth._auth_header == "Basic dGVzdF9rZXlfaWQ6dGVzdF9rZXlfc2VjcmV0"
        
        elif auth_type == HTTPAuthentication.OAUTH.value:
            mock_sinch_async.authentication.get_auth_token.return_value.access_token = "test_token"
            result = await transport.authenticate(endpoint, base_request)
            assert result.headers["Authorization"] == "Bearer test_token"
            assert result.headers["Content-Type"] == "application/json"
    ...
    async def test_authenticate_missing_credentials(self, mock_sinch_async, base_request, auth_type, missing_creds):
        transport = MockAsyncHTTPTransport(mock_sinch_async)
        endpoint = MockEndpoint(auth_type)

        for cred, value in missing_creds.items():
            setattr(mock_sinch_async.configuration, cred, value)

        with pytest.raises(ValidationException):
            await transport.authenticate(endpoint, base_request)