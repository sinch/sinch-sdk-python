...
def endpoint(request_data):
    return GetBatchDetailsEndpoint("test_project_id", request_data)
...
def mock_error_response():
    return HTTPResponse(
        ...
        body={
            ...
            "instance": "/v2/projects/5c5bf2b1-35ae-4825-ab89-457e07bb60e6/batches/01BX5ZZKBKACTAV9WEVGEMMVRC/details",
        },
...
def test_build_url_expects_correct_url(endpoint, mock_sinch_client_voice):
    ...
    assert endpoint.build_url(mock_sinch_client_voice) == (
        "https://voice.api.sinch.com/v2/projects/test_project_id/batches/01BX5ZZKBKACTAV9WEVGEMMVRC/details"
    )
...
def test_handle_response_expects_correct_mapping(endpoint, mock_response):
    """Test that the response is parsed and mapped into a BatchDetailsResponse correctly."""
    parsed_response = endpoint.handle_response(mock_response)

    assert isinstance(parsed_response, BatchDetailsResponse)
    assert len(parsed_response.sessions) == 3
    assert parsed_response.sessions[0].id == "01F8Z5J4X2G9Y3J4X2G9Y3J4X2G9"
    assert parsed_response.sessions[0].state == "COMPLETED"
    assert parsed_response.sessions[1].state == "IN_PROGRESS"
    assert parsed_response.sessions[2].state == "QUEUED"
