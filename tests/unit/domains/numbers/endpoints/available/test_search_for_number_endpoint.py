...
def test_handle_response_expects_correct_mapping(mock_request_data, mock_response):
    ...
    assert response.setup_price == {"currency_code": "USD", "amount": "2.00"}
    assert response.monthly_price == {"currency_code": "USD", "amount": "2.00"}
...
def test_handle_response_expects_missing_fields(mock_response):
    ...
    assert response.monthly_price == {"currency_code": "USD", "amount": "2.00"}
    assert response.supporting_documentation_required is True
    assert "payment_interval_months" not in response.dict()


def test_handle_response_expects_error_on_invalid_response():
    """
    Check if response handling raises an error for invalid response structures.
    """
    invalid_response = HTTPResponse(
        status_code=200,
    ...
    with pytest.raises(ValueError) as excinfo:
        endpoint.handle_response(invalid_response)
